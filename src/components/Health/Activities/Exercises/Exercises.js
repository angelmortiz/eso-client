  ...
  useEffect(() => {
    setFilteredExercises(() => {
      return exercises.filter((ex) => {
        ...
        let muscleResult = selectedMuscle
          ? ex.mainMuscle?.muscleId === selectedMuscle
          : true;
        let equipmentResult = selectedEquipment
          ? ex.equipments[0]?.equipmentId === selectedEquipment
          : true;
  ...
  return (
    <GridView title="Exercises">
      ...
      <section className={styles['filters-section']}>
        ...
        {showFilters && (
          <div className={styles['filters-selects']}>
            ...
            <div className={styles['filter']}>
              <label htmlFor="select-muscle" className={styles['text-label']}>
                Muscle:
              </label>
              <select
                key="select-muscle"
              ...
              >
                {muscleFilterOptions.map((option, index) => {
                  return (
                    <option
                      key={`muscle_${option.name}_${index + 1}`}
                      value={option._id}
                    >
                      {option.name}
                    </option>
            ...
            <div className={styles['filter']}>
              ...
              >
                {equipmentFilterOptions.map((option, index) => {
                  return (
                    <option
                      key={`equipment_${option.name}_${index + 1}`}
                      value={option._id}
                    >
                      {option.name}
                    </option>
            ...
            {/* TYPE FILTER */}
            <div className={styles['filter']}>
              <label htmlFor="select-type" className={styles['text-label']}>
                Type:
              </label>
              <select
                key="select-type"
              ...
              >
                {typeFilterOptions.map((option, index) => {
                  return (
                    <option
                      key={`type_${option.value}_${index + 1}`}
                      value={option.value}
                    >
                      {option.label}
                    </option>
            ...
            <button className={styles['clear-btn']} onClick={clearFilters}>
              Clear filters
            </button>
          </div>
        )}
        {showFilters && (
          <hr
            className={`${styles['separator']} ${styles['separator-bottom']}`}
          />
        )}
      </section>
      {addInfoCards()}
